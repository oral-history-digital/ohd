...
describe MetadataImportTemplate do
  ...
  describe 'basic generation' do
    ...
    it 'should contain all base column-headers' do
      expect(@csv.parse_csv(col_sep: "\t")).to eq(["Interview-ID", "Originalsignatur", "Sprache", "Teilsammlung", "Interview-Datum", "Medientyp", "Dauer", "Protokoll", "Beschreibung", "Anzahl der Bänder", "Link zum Interview", "Vorname", "Nachname", "Geburtsname", "Weitere Namen", "Weitere Vornamen", "Geschlecht", "Geburtsdatum", "Biographie", "Interviewführung", "Transkription", "Übersetzung", "Erschließung"])
    end
  end
