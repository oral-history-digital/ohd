...
describe MetadataExport do
  ...
  describe 'exporting metadata' do
    ...
    it 'should write a csv containing all relevant data' do
      ...
      expect(first_row_entries[3]).to eq(interview.secondary_language.name(:de))
      expect(first_row_entries[4]).to eq(interview.translation_language.name(:de))
      expect(first_row_entries[5]).to eq(interview.collection.name(locale))
      expect(first_row_entries[6]).to eq(interview.interview_date)
      expect(first_row_entries[7]).to eq(interview.media_type)
      expect(first_row_entries[8]).to eq(Timecode.new(interview.duration).timecode)
      expect(first_row_entries[9]).to eq(interview.observations(locale))
      expect(first_row_entries[10]).to eq(interview.description(locale))
      expect(first_row_entries[11]).to eq(interview.tape_count.to_s)
      expect(first_row_entries[12]).to eq(interview.properties[:link])
      expect(first_row_entries[13]).to eq(interview.interviewee.first_name)
      expect(first_row_entries[14]).to eq(interview.interviewee.last_name)
      expect(first_row_entries[15]).to eq(interview.interviewee.birth_name)
      expect(first_row_entries[16]).to eq(interview.interviewee.alias_names)
      expect(first_row_entries[17]).to eq(interview.interviewee.other_first_names)
      expect(first_row_entries[18]).to eq(interview.interviewee.pseudonym_first_name)
      expect(first_row_entries[19]).to eq(interview.interviewee.pseudonym_last_name)
      expect(first_row_entries[20]).to eq(interview.interviewee.use_pseudonym)
      expect(first_row_entries[21]).to eq(interview.interviewee.description(locale))
      expect(first_row_entries[22]).to eq(interview.interviewee.gender)
      expect(first_row_entries[23]).to eq(interview.interviewee.date_of_birth)
      expect(first_row_entries[24]).to eq(interview.interviewee.biography(locale))
      expect(first_row_entries[25]).to eq(interview.interviewee.biography_public?)
      expect(first_row_entries[26]).to eq(interview.interviewers.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[27]).to eq(interview.transcriptors.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[28]).to eq(interview.translators.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[29]).to eq(interview.researches.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[30]).to eq("Deutschland")
      expect(first_row_entries[31]).to eq("Deutschland")
