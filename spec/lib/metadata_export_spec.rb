...
describe MetadataExport do
  ...
  describe 'exporting metadata' do
    ...
    it 'should write a csv containing all relevant data' do
      ...
      expect(first_row_entries[16]).to eq(interview.interviewee.gender)
      expect(first_row_entries[17]).to eq(interview.interviewee.date_of_birth)
      expect(first_row_entries[18]).to eq(interview.interviewee.biography(locale))
      expect(first_row_entries[19]).to eq(interview.interviewers.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[20]).to eq(interview.transcriptors.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[21]).to eq(interview.translators.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[22]).to eq(interview.researches.map{|c| "#{c.last_name}, #{c.first_name}"}.join('#').to_s)
      expect(first_row_entries[23]).to eq("Deutschland")
      expect(first_row_entries[24]).to eq("Deutschland")
