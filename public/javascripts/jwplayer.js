...
(function (b) {
    ...
    b.api.PlayerAPI = function (d) {
        ...
        this.stateListener = function (o, p) {
            if (!n[o]) {
                n[o] = [];
                this.eventListener(b.api.events.JWPLAYER_PLAYER_STATE, f(o))
            }
            n[o].push(p);
            return this
        };
        ...
        this.addInternalListener = function (o, p) {
            o.jwAddEventListener(p, 'function(dat) { jwplayer("' + this.id + '").dispatchEvent("' + p + '", dat); }')
        };
        this.eventListener = function (o, p) {
            if (!j[o]) {
                j[o] = [];
                if (g && i) {
                    this.addInternalListener(g, o)
                }
            }
            j[o].push(p);
            return this
        ...
        this.dispatchEvent = function (q) {
            if (j[q]) {
                var p = e(q, arguments[1]);
                for (var o in j[q]) {
                    if (typeof j[q][o] == "function") {
                        j[q][o].call(this, p)
                    }
                }
            }
        ...
        function e(p, o) {
            var q = b.utils.extend({}, o);
            if (p == b.api.events.JWPLAYER_FULLSCREEN) {
                q.fullscreen = q.message;
                delete q.message
            } else {
                if (typeof q.data == "object") {
                    q = b.utils.extend(q, q.data);
                    delete q.data
                }
            }
            return q
        }
        this.callInternal = function (p, o) {
            if (i) {
                if (typeof g != "undefined" && typeof g[p] == "function") {
                    if (o !== undefined) {
                        return (g[p])(o)
                    } else {
                        return (g[p])()
        ...
        this.playerReady = function (q) {
            i = true;
            if (!g) {
                this.setPlayer(document.getElementById(q.id))
            }
            this.container = document.getElementById(this.id);
            for (var o in j) {
                this.addInternalListener(g, o)
            }
            this.eventListener(b.api.events.JWPLAYER_PLAYLIST_ITEM, function (r) {
                m = {}
            });
            this.eventListener(b.api.events.JWPLAYER_MEDIA_META, function (r) {
                b.utils.extend(m, r.metadata)
            });
            this.dispatchEvent(b.api.events.API_READY);
            while (h.length > 0) {
                var p = h.shift();
                this.callInternal(p.method, p.parameters)
            }
        };
        ...
        this.destroy = function () {
            j = {};
            h = [];
            if (this.container.outerHTML != l) {
                b.api.destroyPlayer(this.id, l)
            }
        };
    ...
    b.api.PlayerAPI.prototype = {
        ...
        getPlaylist: function () {
            return this.callInternal("jwGetPlaylist")
        },
...
(function (a) {
    ...
    a.embed.embedFlash = function (e, i, d) {
        ...
        for(var param in flashParams) {
          if(j[param] != undefined) {
            flashParams[param] = j[param];
            delete j[param];
          }
        }
        /* bug fix end */
        ...
        if (a.utils.isIE()) {
            ...
            f += '<param name="wmode" value="' + flashParams.wmode + '">';
            f += '<param name="allowfullscreen" value=""' + flashParams.allowfullscreen + '">';
            f += '<param name="allowscriptaccess" value=""' + flashParams.allowscriptaccess + '">';
            ...
            a.embed.appendAttribute(h, "wmode", flashParams.wmode);
            a.embed.appendAttribute(h, "allowfullscreen", flashParams.allowfullscreen);
            a.embed.appendAttribute(h, "allowscriptaccess", flashParams.allowscriptaccess);
...
(function (b) {
    ...
    b.html5.utils.getAbsolutePath = function (h) {
        if (h === undefined) {
            return undefined
        }
        if (a(h)) {
        ...
        var j = document.location.href.substr(0, document.location.href.indexOf("://") + 3);
        var f = document.location.href.split("?")[0];
        f = f.substring(j.length, (h.indexOf("/") === 0) ? f.indexOf("/", j.length) : f.lastIndexOf("/"));
        var e = (f + "/" + h).split("/");
        var d = [];
...
(function (a) {
    ...
    a.html5.mediavideo = function (f, C) {
        ...
        function l(P) {
            ...
            if (P.target.error || P.target.parentNode.error) {
