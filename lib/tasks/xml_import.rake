namespace :xml_import do
  ...
  desc "checks interview quality and sets the flag accordingly if an import was expected in the meantime"
  task :quality, [:id] => :environment do |task,args|
    require 'nokogiri'

    id = args[:id]

    puts "\nChecking and updating quality for #{id.blank? ? 'all interviews' : id}:"
    repo_dir = File.join(ActiveRecord.path_to_storage, ARCHIVE_MANAGEMENT_DIR)
    if id.blank?
      # all interviews
      repo_dir = File.join(repo_dir, 'za**')
    ...
    Dir.glob(File.join(repo_dir, 'data', 'za*.xml')).each do |xmlfile|
      ...
      next if archive_id.blank?
      puts archive_id
      @parser = Nokogiri::XML::SAX::Parser.new(QualityXMLImport.new(xmlfile))
      @parser.parse(File.read(xmlfile))
      puts
    end
