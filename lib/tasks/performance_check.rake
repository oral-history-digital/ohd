...
namespace :performance do
  ...
  def performance_user_and_token(project)
    email = ENV["PERF_USER"] || "alice@example.com"
  ...
  #   bin/rake performance:check
  #
  #   # Run a single endpoint by name (matches keys in the `endpoints` hash)
  #   ENDPOINT=archive_search bin/rake performance:check
  ...
  #   PROJECT=za LOCALE=de bin/rake performance:check
  #
  #   # Show DB queries and verbose output
  #   SHOW_QUERIES=true VERBOSE=true bin/rake performance:check
  #
  #   # Provide a custom perf user email (must exist) and timeout (seconds)
  #   PERF_USER=alice@example.com TIMEOUT=120 bin/rake performance:check
  #
  #   # Use an existing OAuth token instead of creating one
  #   PERF_TOKEN=abcdef... bin/rake performance:check
  #
  #   # Test a specific registry entry by ID
  #   PROJECT=za REGISTRY_ENTRY_ID=90436993 bin/rake performance:check[registry_entry_detail]
  #
  # Notes:
  ...
  #   - `REGISTRY_ENTRY_ID` defaults to the first registry entry in the project.
  ...
  task :check, [:endpoint] => :environment do |_, args|
    ...
    # Allow specifying a specific registry entry ID, otherwise use first available
    registry_entry = if ENV["REGISTRY_ENTRY_ID"]
      project.registry_entries.find_by(id: ENV["REGISTRY_ENTRY_ID"])
    else
      project.registry_entries.first
    end
    ...
    endpoints = {
      ...
      registry_entry_detail: registry_entry && {
        label: "GET /:pid/registry_entries/:id.json?with_associations=true",
        path: "#{base_path}/registry_entries/#{registry_entry.id}.json",
        params: { with_associations: true }
      },
