...
namespace :stats do
  namespace :users do
    ...
    task :list, [:criteria, :sort, :skip] => :environment do |t, args|
      # Disable SQL logging for cleaner output
      old_logger = ActiveRecord::Base.logger
      ...
      when 'empty_name'
        ...
        puts "\nUsage: bin/rake stats:users:list[criteria,sort,skip]"
        puts "Example: bin/rake stats:users:list[never_logged_in,created_at_asc,0]"
        puts "Example: bin/rake stats:users:list[unconfirmed,email_asc,100]  # skip first 100"
        next
      end
