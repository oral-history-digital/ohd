...
class LocationsXMLImport < Nokogiri::XML::SAX::Document
  ...
  def start_element(name, attributes={})
    ...
    @current_element = name if @parsing_location
  end
  ...
  def end_element(name)
    if @parsing_location && %w(location locations).include?(name)
      @parsing_location = false
    end
    if @parsing_location && name == 'name'
    ...
    @current_language = nil unless @parsing_location
    @current_data = ''
    @current_element = nil
  end
