...
public class Segments extends MovieClip implements PluginInterface {
	...
	private function itemHandler(evt:ControllerEvent=null):void {
		...
		field.htmlText = '';
		var file:String;
		if (view.playlist[view.config['item']]['segments.file']){
			file = view.playlist[view.config['item']]['segments.file'];
		} else if (view.playlist[view.config['item']]['segments']){
			file = view.playlist[view.config['item']]['segments']; 
		} else if(view.config['segments.file']) {
			file = view.config['segments.file'];
		} else if(view.config['segments']) {
			file = view.config['segments'];
		}
		if(file) {
			config['file'] = file;
			try {
				...
				Logger.log(err.message,'segments');
			}
	...
	private function loaderHandler(evt:Event):void {	
	  var parser:Object = SegmentsParser.parseSegments(XML(evt.target.data));
		...
		if(segments.length == 0) {
			Logger.log('Not a valid TimedText or SRT file.','segments');
		}
	};
	...
	private function metaHandler(evt:ModelEvent):void {
		var txt:String;
		var fnd:Boolean;
		if(evt.data.type == 'segment') {
			txt = evt.data.segments;
			fnd = true;
	...
	private function setSegment(pos:Number):void {
		for(var i:Number=0; i<segments.length; i++) {
			if(segments[i]['begin'] < pos && (i == segments.length - 1 || segments[i+1]['begin'] > pos)) {
				current = i;
				/*
	...
	private function timeHandler(evt:ModelEvent):void {
		...
		if(segments && segments.length > 0 && (
			segments[current]['begin'] > pos || 
			(segments[current+1] && segments[current+1]['begin'] < pos))) {
			setSegment(pos);
		}
	};
