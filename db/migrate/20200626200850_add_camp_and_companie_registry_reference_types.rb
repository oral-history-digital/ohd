class AddCampAndCompanieRegistryReferenceTypes < ActiveRecord::Migration[5.2]
  def change
    if Project.current.identifier.to_sym == :zwar
      ...
      RegistryReference.where(registry_entry_id: camp.all_relatives.map(&:id).flatten).update_all(registry_reference_type_id: camp_type.id)
      RegistryReference.where(registry_entry_id: companie.all_relatives.map(&:id).flatten).update_all(registry_reference_type_id: companie_type.id)
    end
  end
end
