class ConvertRegistryEntryMetadataFieldsToRegistryReferenceTypeMetadataFields < ActiveRecord::Migration[5.2]
  def change
    ...
    MetadataField.where(source: "RegistryEntry").each do |metadata_field|
      ...
      registry_entry = RegistryEntry.find_by_code(original_name)
      if registry_entry
        ...
        registry_entry.children.map(&:id).map{|id|RegistryReference.where(registry_entry_id: id)}.flatten.each do |rr|
