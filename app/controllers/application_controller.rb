  ...
  def initial_redux_state
    {
      archive: {
        ...
        registryEntryMetadataFields: current_project.registry_entry_metadata_fields.where("use_in_details_view").map { |field| { id: RegistryEntry.find_by_code(field.name).id, code: RegistryEntry.find_by_code(field.name).code, display_on_landing_page: field.display_on_landing_page } },
        registryReferenceTypeMetadataFields: current_project.registry_reference_type_metadata_fields.inject({}) { |mem, facet| mem[facet.name] = RegistryReferenceTypeSerializer.new(RegistryReferenceType.find_by_code(facet.name)).as_json; mem },
      },
      account: {
        isLoggingIn: false,
