    ...
    respond_to do |format|
      format.json do
        render json: {
          ...
          data: Rails.cache.fetch("#{Project.current.cache_key_prefix}-registry_entry-#{descendant.id}-#{descendant.updated_at}"){::RegistryEntrySerializer.new(descendant).as_json},
