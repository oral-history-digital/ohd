class HistoriesController < ApplicationController
  ...
  def clear_person_cache(person)
    Rails.cache.delete "#{Project.current.cache_key_prefix}-person-#{person.id}-#{person.updated_at}"
  end

  def clear_history_cache id, updated_at
    Rails.cache.delete "#{Project.current.cache_key_prefix}-history-#{id}-#{updated_at}"
  end
