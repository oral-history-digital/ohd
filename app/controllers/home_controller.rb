        ...
        json = Rails.cache.fetch("#{Project.cache_key_prefix}-static-content") do
          {
            ...
            registry_entry_search_facets: Project.registry_entry_search_facets.map { |facet| { id: RegistryEntry.find_by_entry_code(facet["id"]).id, display_on_landing_page: facet["display_on_landing_page"], label: facet["facet_label"] } },
            metadata_fields_registry_reference_type: Project.metadata_fields_registry_reference_type.inject({}) { |mem, facet| mem[facet["id"]] = RegistryReferenceTypeSerializer.new(RegistryReferenceType.find_by_code(facet["id"])).as_json; mem },
