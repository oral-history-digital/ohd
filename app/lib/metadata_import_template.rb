class MetadataImportTemplate
  ...
  def default_interview_columns
    [
      ...
      mem[c] = TranslationValue.for("metadata_labels.#{c}", @locale).chop
      mem
    end
  end
  ...
  def default_interviewee_columns
    [
      ...
      mem[c] = TranslationValue.for("activerecord.attributes.person.#{c}", @locale).chop
      mem
    end
  end
  ...
  def default_contributor_columns
    @project.contribution_types.where(use_in_export: true).inject({}) do |mem, c| 
      mem[c.code.to_sym] = TranslationValue.for("contributions.#{c.code}", @locale).chop
      mem
    end
  end
