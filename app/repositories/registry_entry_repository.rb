class RegistryEntryRepository
  ...
  def combined_entries_for_map()
    result = RegistryEntry
      .select('reg_with_counts.id, re.latitude, re.longitude, reg_with_counts.ref_type_counts, JSON_OBJECTAGG(rnt.locale, rnt.descriptor) AS agg_names')
  ...
  def merged_reg_entries()
    result = RegistryEntry
      .select('merged.id, JSON_ARRAYAGG(merged.ref_type_counts)')
