class TextMaterial < ApplicationRecord
  ...
  scope :for_file, -> (filename) { where('document_file_name = ?', (filename || '') + '.pdf') }
  ...
  #def document_file_name=(filename)
    ...
    #filename = (filename || '').sub!(/\w{3,4}$/,'pdf')
      ...
      #archive_id = ((filename || '')[Regexp.new("^#{Project.current.shortname}\\d{3}", Regexp::IGNORECASE)] || '').downcase
      ...
      #filepath = if !interview.nil? and interview.quality < 2.0
        ...
        #versions_dir = File.join(Project.repository_dir, archive_id.upcase, archive_id.upcase + '_archive', 'versions', 'bm', (filename || '').split('/').last.to_s[Regexp.new("#{Project.current.shortname.downcase}\\d{3}_\\w+/")])
        #ctime = Time.now
        #original_file = nil
