...
class Annotation < ActiveRecord::Base
  ...
  def localized_hash
    I18n.available_locales.inject({}) do |mem, locale|
      mem[locale] = text(locale) if Project.available_locales.include?( locale.to_s )
      #mem[locale] = ActionView::Base.full_sanitizer.sanitize(text(locale)).gsub("&nbsp;", " ")  if Project.available_locales.include?( locale.to_s )
      mem
    end
  end
