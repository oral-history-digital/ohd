module Interview::OaiDc
  ...
  def to_oai_dc
    ...
    xml.tag!(
      ...
      xml.tag!('oai_dc:identifier', oai_identifier)

      oai_locales.each do |locale|
        xml.tag!('oai_dc:identifier', "xml:lang": locale, identfierType: "URL") do
          xml.text! oai_url_identifier(locale)
        end
      end

      xml.tag!('oai_dc:identifier', identfierType: "DOI") do
        xml.text! oai_doi_identifier
      end

      oai_locales.each do |locale|
        xml.tag!('oai_dc:title', "xml:lang": locale) do
      ...
      interviewees.each do |interviewee|
        oai_locales.each do |locale|
          xml.tag!('oai_dc:creator', "xml:lang": locale) do
      ...
      oai_locales.each do |locale|
        xml.tag!('oai_dc:publisher', "xml:lang": locale) do
          xml.text! oai_publisher(locale)
        end
      end

      xml.tag!('oai_dc:date', oai_date)

      xml.tag!('oai_dc:type', oai_type)

      xml.tag!('oai_dc:format', oai_format)

      xml.tag!('oai_dc:language', oai_language)

      oai_subject_registry_entry_ids.each do |registry_entry_ids|
        unless registry_entry_ids.empty?
          oai_locales.each do |locale|
            xml.tag!('oai_dc:subject', "xml:lang": locale) do
      ...
      oai_locales.each do |locale|
        xml.tag!('oai_dc:rights', "xml:lang": locale) do
