module Segment::EncipheredText
  ...
  def enciphered_text(version, locale)
    ...
    previous_text = nil
    current_text = text_original.dup

    while current_text != previous_text
      previous_text = current_text.dup
      current_text = encipher_step(version, current_text, hidden_text)
    end
    current_text
  end

  def encipher_step(version, text_original, hidden_text)
    enciphered = case version
    when :subtitle
      text_original.
        # colonia
