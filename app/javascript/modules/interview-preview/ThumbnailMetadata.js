...
export default function ThumbnailMetadata({ interview }) {
    const { project } = useProject();
    const { locale } = useI18n();
    const { humanReadable } = useHumanReadable();
    const { data: interviewee, isLoading } = usePersonWithAssociations(
        ...
        <ul className="DetailList" lang={locale}>
            {!project.grid_fields || isLoading
                ...
                : project.grid_fields.map((field) => {
                      ...
                      const obj =
                          field.ref_object_type === 'Interview' ||
                          field.source === METADATA_SOURCE_INTERVIEW
                              ? interview
                              : interviewee;

                      if (field.source === METADATA_SOURCE_EVENT_TYPE) {
                          const events = interviewee?.events?.filter(
                              (e) => e.event_type_id === field.event_type_id
                          );
                          ...
                          return (
                              <li key={field.name} className="DetailList-item">
                          ...
                          return (
                              <li
                                  ...
                                  className={classNames('DetailList-item', {
                                      'DetailList-item--shortened':
                                          field.name === 'description',
                                      'DetailList-item--one-line':
