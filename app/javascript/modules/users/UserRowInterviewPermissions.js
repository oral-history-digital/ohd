...
export default function UserRowActions({ row, getValue }) {
    ...
    const currentProjectInterviewPermissions =
        row.original.interview_permissions?.filter((permission) => {
            return interviews?.some(
                (interview) => interview.id === permission.interview_id
            );
        });
    const none =
        !currentProjectInterviewPermissions ||
        currentProjectInterviewPermissions.length === 0;
    const names = currentProjectInterviewPermissions
        ?.sort((a, b) =>
    ...
    return interviews?.length === 0 ? null : (
        <>
            <span>
                {!none &&
                    `${currentProjectInterviewPermissions.length} ${t('activerecord.models.interview_permission.other')}: `}
            </span>
            <span>
                {!none && names}
                {hasMore && '...'}
