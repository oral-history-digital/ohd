...
export default function JoinedData({
    joinedData,
    data,
    scope,
}) {
    ...
    return Object.keys(joinedData).map((joined_model_name_underscore, index) => {
        const props = {
        ...
        return (
            <div
                className={classNames(pluralize(joined_model_name_underscore), 'box')}
                key={joined_model_name_underscore}
            >
                <h4 className='title'>
                    {t(`activerecord.models.${joined_model_name_underscore}.other`)}
                </h4>
                {createElement(joinedData[joined_model_name_underscore], props)}
            </div>
        );
    });
}
