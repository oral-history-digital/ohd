...
export default function FacetValues({ data, facet, filter, locale }) {
    ...
    function sortedSubfacets() {
        ...
        if (data.name['de'] && data.name['de'].trim() === 'Zeitperioden') {
            return Object.keys(data.subfacets).sort((a, b) => {
                return localDescriptor(a).replace(/[^\d]/g, '') >
                    localDescriptor(b).replace(/[^\d]/g, '')
                    ? 1
                    : localDescriptor(b).replace(/[^\d]/g, '') >
                        localDescriptor(a).replace(/[^\d]/g, '')
                      ? -1
                      : 0;
            });
        }
        ...
        // sort first alphabetically, then put prioritized down in the list (like "others"/"sonstige")
            ...
            return Object.keys(data.subfacets)
                .sort((a, b) => {
                    return localDescriptor(a) > localDescriptor(b)
                        ? 1
                        : localDescriptor(b) > localDescriptor(a)
                          ? -1
                          : 0;
                })
                .sort((a, b) => {
    ...
    return sortedSubfacets()
        ...
        .map((subfacetId) => {
            ...
            if (checkedFacets) {
                checkedState =
                    checkedFacets.indexOf(subfacetId.toString()) > -1;
            }
            return (
                <FacetValue
                    key={subfacetId}
