class ReadTranscriptFileJob < ApplicationJob
  ...
  def perform(interview, file_path, tape_id, file_column_names, file_column_languages, receiver)
    ...
    when 'ods', 'xlsx', 'xlsm','xls', 'xlm', 'csv'
      interview.create_or_update_segments_from_spreadsheet(file_path, tape_id, file_column_names, file_column_languages)
    when 'vtt', 'srt'
      interview.create_or_update_segments_from_vtt(file_path, tape_id)
    end
